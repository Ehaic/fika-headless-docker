...
- [📦 Releases](#-releases)
- [🚤 Running](#-running)
    + [NarcoNet mod sync](#narconet-mod-sync)
...
- 🔄 Supports [NarcoNet](https://gitlab.com/flir063-spt/narconet/) mod sync for SPT 4.0.x/4.1.x (recommended — runs natively on Linux, no Wine needed)
...
- 🧪 Tested and works on SPT 3.9.x, 3.10.x, 3.11.x, 4.0.x (4.1.x support via the [sp-tushonka server image](https://github.com/Ehaic/fika-spt-server-docker))
...
## NarcoNet mod sync
[NarcoNet](https://gitlab.com/flir063-spt/narconet/) keeps the headless client's mods and configs in lockstep with the server, and is the recommended sync method for SPT 4.0.x and 4.1.x. Unlike Corter-ModSync, its syncer runs as a native Linux binary before the game starts — no Wine, no Xvfb tricks.

### Prerequisites
- The NarcoNet **client plugin** must be present in the client's `BepInEx/plugins/MadManBeavis-NarcoNet/` folder, including the bundled `narconetsync-linux-*` binary.
  - On first connect, the server's `headless` sync profile will serve the plugin folder to the client — so you can also just connect once with `USE_NARCONET` unset, let NarcoNet install itself, then enable it.
  - Or download the [latest release zip](https://gitlab.com/api/v4/projects/80043430/packages/generic/NarcoNet/1.9.0/NarcoNet-v1.9.0.zip), extract it into the client install, and restart.
- The server's `config.yaml` must define a `headless` sync profile (a config generated by NarcoNet ships one you can copy). The syncer **refuses** to sync a headless client if the server did not apply the profile — this is deliberate, since syncing the base configuration would deliver player-only mods that break raid hosting.

### Enabling
Set `USE_NARCONET=true` and point `SERVER_URL`/`SERVER_PORT` at your server (the example compose already does). The container then:

1. Runs the native syncer before **every** client start (including after raid-end auto-restarts), while no files are held open
2. Auto-infers the `headless` profile from the Fika headless plugin on disk (or set `NARCONET_PROFILE` explicitly)
3. Auto-resolves the server from `SERVER_URL`/`SERVER_PORT` (TLS verification is off by default — SPT uses a self-signed cert)

### Configuration
All syncer options can be passed as environment variables (`NARCONET_PROFILE`, `NARCONET_DELETE_REMOVED`, `NARCONET_SYNC_ARGS` for extra CLI flags, etc. — see `narconetsync --help`). The plugin folder is a sync path, so the server keeps both the in-game plugin and the syncer binary updated after initial setup.
...
| `USE_NARCONET`                 | If set to `true`, runs the native NarcoNet syncer before every client start (recommended for SPT 4.0.x/4.1.x). See [NarcoNet mod sync](#narconet-mod-sync) |
| `NARCONET_PROFILE`             | Sync as this profile. Defaults to `headless` |
| `NARCONET_SYNC_BIN`            | Override path to the `narconetsync-linux-*` binary. By default it is auto-discovered under `BepInEx/plugins` |
| `NARCONET_SYNC_ARGS`           | Extra CLI arguments passed to the syncer (e.g. `--delete-removed`). All syncer options can also be set via their own `NARCONET_*` environment variables — see `narconetsync --help` |
